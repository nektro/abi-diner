...
pub fn main() !void {
    ...
    switch (std.meta.stringToEnum(enum { caller, callee }, args.next().?).?) {
        .caller => {
            ...
            try writer.writeAll("\n");
            try writer.writeAll("extern void do_test(");
            try renderType(tag, writer);
            try writer.writeAll(");\n");
            try writer.writeAll("void do_caller(void) {\n");
            try writer.writeAll("    ");
